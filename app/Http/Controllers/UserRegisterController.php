      ...
      else
      {
          ...
          if (!empty($email))
          {
              ...
              if ($isExists == '') 
              {
                  ...
                  $success['userid']               = $GetMessageDetail->id;
                  $success['useremail']            = $GetMessageDetail->email;
                  $success['username']             = $GetMessageDetail->name;
                  $success['paypal_email']         = $getPayPalEmail;
                  $success['notification_count']   = $NotificationCount;
                  $success['error_code']           = "0";
                  $success['msg']                  =  'Registration completed successfully.';
                  return response()->json(['data'=>$success]);
              }
              elseif($isExists->status == '1')
              {
                  $success['msg'] =  'Email Already Exist. Please use other email address.';
