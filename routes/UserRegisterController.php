...
class UserRegisterController extends Controller
{
    ...
    {
        $NewPassword = User::where('id', $request->userId)
                                ->where('email',$request->useremail)->first();
        if (isset($NewPassword) && count($NewPassword) > 0 ) 
        {
            $ChangePassword->update(['status' => 1, 'password' => bcrypt($request->pwd)]);
            
        }
        return response()->json(["success"=>true,
            "msg"=>"Please Enter a strong password.",'error_code' => '5']);
    }
    public function ConformPassword(Request $request)
    {
        $ConformPassword = User::where('id', $request->userId)
                                ->where('email',$request->useremail)->first();
        if (isset($ConformPassword) && count($ConformPassword) > 0 ) 
        {
            $ConformPassword->update(['status' => 1, 'password' => bcrypt($request->pwd)]);
